...
class JuliaExternalModulePlugins:
    ...
    def visit_response_get(
        self, node: ast.Call, vargs: list[str], kwargs: list[tuple[str, str]]
    ):
        self._usings.add("HTTP")
        return f"String({vargs[0]}.body)"

    def visit_response_status_code(
        self, node: ast.Call, vargs: list[str], kwargs: list[tuple[str, str]]
    ):
        self._usings.add("HTTP")
        return f"{vargs[0]}.status"

    def visit_http_error(
        self, node: ast.Call, vargs: list[str], kwargs: list[tuple[str, str]]
    ):
        self._usings.add("HTTP")
        return (
            f"HTTP.Exceptions.StatusError"
            if not vargs
            else f"HTTP.Exceptions.StatusError({', '.join(vargs)})"
        )
...
SMALL_DISPATCH_MAP = {"requests.codes.ok": lambda node, vargs, kwargs: "200"}
...
IGNORED_MODULE_SET = set(["requests"])
...
EXTERNAL_TYPE_MAP = {
    requests.HTTPError: lambda self: JuliaExternalModulePlugins.visit_http_error(
        self, None, [], []
    )
}
