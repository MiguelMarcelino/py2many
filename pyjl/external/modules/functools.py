...
class JuliaExternalModulePlugins:
    def visit_partial(
        ...
        return (
            f"functools.partial({', '.join(vargs)})" if vargs else "functools.partial"
        )
...
IGNORED_MODULE_SET = set(["functools"])

EXTERNAL_TYPE_MAP = {
    functools.partial: lambda self: JuliaExternalModulePlugins.visit_partial(
        self, None, [], []
    ),
}
