...
class InferJuliaTypesTransformer(InferTypesTransformer, ExternalBase):
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Attribute) and node.func.attr == "append":
            ...
            if (
                ...
                and get_id(ann.value) in {"List", "list"}
                and arg_ann
                and get_id(arg_ann) != "Any"
                and get_id(arg_ann) != get_id(ann.slice)
            ):
        ...
        for target in node.targets:
            ...
            if hasattr(node, "annotation"):
                self._verify_annotation(
                    node, getattr(target, "annotation", None), target, inferred=inferred
                )
        return node
    ...
    def _get_import_type(self, node):
        ...
        mod_name = None
        check_imports = lambda x: x in self._imported_names and self._imported_names[x]
        get_mod_name = (
            lambda x: x[0]
            if isinstance(x, tuple)
            else (x.__name__ if hasattr(x, "__name__") else x)
        )
        ...
        if mod_name:
            ...
            is_dir_or_module = is_dir(f"{mod_name}", self._basedir) or is_file(
                f"{mod_name}", self._basedir
            )
            if is_dir_or_module:
    ...
    def visit_BinOp(self, node):
        ...
        is_numeric = (
            ...
            or "complex"
            or (
                isinstance(x, str) and (x.startswith("c_int") or x.startswith("c_uint"))
            )
        )
        ...
        if (
            ...
            and (left_id, right_id, type(node.op)) in ILLEGAL_COMBINATIONS
        ):
    ...
    def _verify_annotation(self, node, annotation, target, inferred=False):
        ...
        if (
            map_ann_str
            and not getattr(map_ann, "is_inferred", True)
            and ann_str != self._default_type
            and map_ann_str != self._default_type
            and ann_str.lower() != map_ann_str.lower()
        ):
            raise AstIncompatibleAssign(
                f"Variable {var_name}: {ann_str} incompatible with {map_ann_str}", node
            )
    ...
    def _assign_annotation(self, node, *defaults):
        if isinstance(node, ast.BinOp):
            ...
            if get_id(node.left):
                left_ann = getattr(
                    node.scopes.find(get_id(node.left)), "annotation", None
                )
            if get_id(node.right):
                right_ann = getattr(
                    node.scopes.find(get_id(node.right)), "annotation", None
                )

            # Assign left and right annotations
            node.left.annotation = left_ann if left_ann else left_default
            node.right.annotation = right_ann if right_ann else right_default
