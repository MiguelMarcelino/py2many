...
class JuliaExternalModulePlugins:
    ...
    def _get_python_dll_path(version: tuple[str, str]):
        ...
        python_path = (
            subprocess.check_output(f"where python{version_str}.dll")
            .decode()
            .strip()
            .split("\n")
        )
        python_path = python_path[1] if len(python_path) > 1 else python_path[0]
    ...
    def visit_cdata_value(
        ...
        if (
            ...
            and get_id(node.args[0].func) == "ctypes.cast"
        ):
    ...
    # Using Pycall
    def visit_create_unicode_buffer(
        self, node: ast.Call, vargs: list[str], kwargs: list[tuple[str, str]]
    ):
...
GENERIC_DISPATCH_TABLE = {
    ...
    ctypes.sizeof: (
        lambda self, node, vargs, kwargs: f"sizeof({self._map_type(vargs[0])})"
        if vargs
        else "sizeof",
        True,
    ),
    ...
    ctypes.create_unicode_buffer: (
        JuliaExternalModulePlugins.visit_create_unicode_buffer,
        True,
    ),
}
...
IGNORED_MODULE_SET = set(["ctypes", "ctypes.wintypes"])
...
if sys.platform.startswith("win32"):
    ...
    WIN_DISPATCH_TABLE = {
        ...
        ctypes.windll.LoadLibrary: (
            JuliaExternalModulePlugins.visit_load_library,
            True,
        ),
        ...
        ctypes.FormatError: (
            lambda self, node, vargs, kwargs: f"Base.Libc.FormatMessage({', '.join(vargs)})",
            True,
        ),
