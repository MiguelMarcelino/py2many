...
class JuliaExternalModulePlugins:
    def visit_basegeometry(
        ...
        return (
            f"shapely_geo_base.BaseGeometry({', '.join(vargs)})"
            if vargs
            else "shapely_geo_base.BaseGeometry"
        )
    ...
    def visit_point(
        ...
        return (
            f"shapely_geo.Point({', '.join(vargs)})" if vargs else "shapely_geo.Point"
        )
...
if shapely:
    ...
    EXTERNAL_TYPE_MAP = {
        BaseGeometry: lambda self: JuliaExternalModulePlugins.visit_basegeometry(
            self, None, [], []
        ),
