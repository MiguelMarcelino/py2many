...
class JuliaMethodCallRewriter(ast.NodeTransformer):
    ...
    def visit_Module(self, node: ast.Module) -> Any:
        ...
        self._oop_nested_funcs = getattr(
            node, OOP_NESTED_FUNCS, FLAG_DEFAULTS[OOP_NESTED_FUNCS]
        )
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(fname, ast.Attribute):
            ...
            if (
                val_id
                and (is_class_type(val_id, node.scopes) or re.match(r"^self", val_id))
            ...
            is_static_access = (
                is_class_or_module(val_id, node.scopes)
                and class_node
                and find_node_by_name_and_type(
                    fname.attr, ast.FunctionDef, class_node.scopes
                )[1]
            )
...
class JuliaAugAssignRewriter(ast.NodeTransformer):
    ...
    def visit_AugAssign(self, node: ast.AugAssign) -> Any:
        ...
        is_class = is_class_type(get_id(node.target), node.scopes) or is_class_type(
            get_id(node.value), node.scopes
        )
        # Template for call node
        call = ast.Call(
            func=ast.Name(id=None, lineno=node.lineno, col_offset=node.col_offset),
            args=[],
            keywords=[],
            lineno=node.lineno,
            col_offset=node.col_offset,
        ...
        if isinstance(node.target, ast.Subscript) and isinstance(
            ...
            if self._is_number(node.value) and isinstance(node.op, ast.Mult):
                ...
                if (
                    ...
                    lower_slice = ast.Constant(
                        value=int(upper.value) + 1, scopes=lower.scopes
                    )
                else:
                ...
                return call
        elif isinstance(node.target, ast.Name) and self._is_collection(node.target):
            if isinstance(node.op, ast.Add):
                call.func.id = "append!"
                call.args.append(node.target)
                call.args.append(node.value)
                return call
            elif isinstance(node.op, ast.Mult) and self._is_number(node.value):
                ...
                if isinstance(node.value, ast.Constant):
                    ...
                    value = ast.BinOp(
                        left=node.value, op=ast.Sub(), right=ast.Constant(value=1)
                    )
    ...
    def _is_number(node):
        return (
            ...
            or (isinstance(node, ast.Constant) and node.value.isdigit())
            or (
                get_id(getattr(node, "annotation", None))
                in InferTypesTransformer.FIXED_WIDTH_INTS
            )
        )
...
class JuliaGeneratorRewriter(ast.NodeTransformer):
    ...
    def visit_Module(self, node: ast.Module) -> Any:
        ...
        self._replace_calls = {}
        # Get flags
        self._use_resumables = getattr(
            node, USE_RESUMABLES, FLAG_DEFAULTS[USE_RESUMABLES]
        )
        self._lower_yield_from = getattr(
            node, LOWER_YIELD_FROM, FLAG_DEFAULTS[LOWER_YIELD_FROM]
        )
    ...
    def visit_YieldFrom(self, node: ast.YieldFrom) -> Any:
        ...
        if isinstance(parent, ast.FunctionDef):
            ...
            lower_yield_from = (
                dec and dec["lower_yield_from"]
            ) or self._lower_yield_from
            if lower_yield_from:
                val = ast.Name(
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if self._sweep:
            ...
            if get_id(node.func) in self.SPECIAL_FUNCTIONS and isinstance(
                ...
                if (
                    func_def
                    and RESUMABLE in func_def.parsed_decorators
                    and parent
                    and hasattr(parent, "n_body")
                ):
    ...
    def visit_Assign(self, node: ast.Assign) -> Any:
        ...
        name = (
            get_id(node.value.value)
            if (isinstance(node.value, ast.Attribute) and node.value.attr == "__next__")
            else get_id(node.value)
        )
        func_def = get_func_def(node, name)
        if (
            func_def
            and get_id(getattr(func_def, "annotation", None)) == "Generator"
            and RESUMABLE not in func_def.parsed_decorators
        ):
...
class JuliaIndexingRewriter(ast.NodeTransformer):
    ...
    RESERVED_FUNCTIONS = set(["__dict__"])
    ...
    def visit_Subscript(self, node: ast.Subscript) -> Any:
        ...
        is_usub = lambda x: (isinstance(x, ast.UnaryOp) and isinstance(x.op, ast.USub))
        end_val = ast.Name(
            id="end", annotation=ast.Name(id="int"), preserve_keyword=True
        )
        if is_usub(node.slice):
            ...
            and is_usub(node.slice.left)
            and get_id(node.slice.left.operand) in self._valid_loop_vars
        ):
        ...
        if not self._is_dict(node) and not isinstance(node.slice, ast.Slice):
            call_id = None
            if isinstance(node.slice, ast.Call):
                call_id = self._get_assign_value(node.slice)

            if not getattr(node, "range_optimization", None) and not getattr(
                node, "using_offset_arrays", None
            ):
                # Ignore special functions, as they already return the correct indices
                if call_id in self.SPECIAL_FUNCTIONS and call_id in self._imports:
                    return node
                if (
                    isinstance(node.value, ast.Attribute)
                    and node.value.attr in self.RESERVED_FUNCTIONS
                ):
                    return node
                ...
                if isinstance(node.slice, ast.Constant) and isinstance(
                    ...
                    if get_id(node.slice) != "end":
                        node.slice = self._do_bin_op(
                            node.slice, ast.Add(), 1, node.lineno, node.col_offset
                        )
            elif getattr(node, "range_optimization", None) and not getattr(
                node, "using_offset_arrays", None
            ):
                ...
                if for_node:
                    ...
                    if (
                        ...
                        and not self._bin_op_contains(node.slice, target_id)
                    ):
                        node.slice = self._do_bin_op(
                            node.slice, ast.Add(), 1, node.lineno, node.col_offset
                        )
                ...
                if call_id in self.SPECIAL_FUNCTIONS and call_id in self._imports:
                    ...
                    assign_node = find_node_by_name_and_type(
                        get_id(node.value), ast.Assign, node.scopes
                    )[0]
                    if (
                        assign_node
                        and isinstance(assign_node.value, ast.Call)
                        and get_id(assign_node.value.func) == "OffsetArray"
                    ):
                        dec = assign_node.value.args[1]
                        dec = -dec.value if dec.value < 0 else dec.value
                        node.slice = self._do_bin_op(
                            node.slice, ast.Sub(), dec, node.lineno, node.col_offset
                        )

        return node

    def _bin_op_contains(self, bin_op: ast.BinOp, node_id):
        if (get_id(bin_op.left) == node_id) or (get_id(bin_op.right) == node_id):
            return True
        contains = False
    ...
    def visit_Slice(self, node: ast.Slice) -> Any:
        ...
        if (
            ...
            isinstance(node.lower, ast.UnaryOp)
            ...
            if (
                isinstance(node.lower.operand, ast.Constant)
                and node.lower.operand.value != 1
            ):
                right = self._do_bin_op(
                    node.lower.operand, ast.Add(), 1, node.lineno, node.col_offset
                )
                ...
                node.lower = length
        elif not getattr(node, "range_optimization", None) and not getattr(
            node, "using_offset_arrays", None
        ):
            if isinstance(node.lower, ast.Constant) and isinstance(
                node.lower.value, int
            ):
                node.lower.value += 1
            elif node.lower:
                # Default: add 1
                node.lower = self._do_bin_op(
                    node.lower, ast.Add(), 1, node.lineno, node.col_offset
                )
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if call_id == "range" or call_id == "xrange":
            ...
            if getattr(node, "range_optimization", None) and not getattr(
                node, "using_offset_arrays", None
            ):
                if len(node.args) == 1:
                    ...
                    node.args[0] = ast.Constant(
                        value=1, scopes=getattr(node.args[0], "scopes", ScopeList())
                    )
                elif len(node.args) > 1:
                    # increment start
                    node.args[0] = self._do_bin_op(
                        node.args[0], ast.Add(), 1, node.lineno, node.col_offset
                    )
            else:
                # decrement stop
                if len(node.args) == 1:
...
class JuliaIORewriter(ast.NodeTransformer):
    ...
    def visit_Subscript(self, node: ast.Subscript) -> Any:
        ...
        if (
            isinstance(node.value, ast.Attribute)
            and get_id(node.value) == "sys.argv"
            ...
            node.slice = ast.BinOp(
                left=node.slice, op=ast.Sub(), right=ast.Constant(value=1)
            )
...
class JuliaOrderedCollectionRewriter(ast.NodeTransformer):
    ...
    def visit_Dict(self, node: ast.Dict) -> Any:
        self.generic_visit(node)
        if (
            getattr(node, "use_ordered_collection", None)
            or self._use_ordered_collections
        ):
            return juliaAst.OrderedDict(
                keys=node.keys, values=node.values, annotation=node.annotation
            )
        return node
    ...
    def visit_Set(self, node: ast.Set) -> Any:
        ...
        if (
            ...
            return juliaAst.OrderedSet(elts=node.elts, annotation=node.annotation)
        return node
...
class JuliaArbitraryPrecisionRewriter(ast.NodeTransformer):
        ...
        if get_id(node) in self._arbitrary_precision_vars:
            ...
            if ann == "int":
                if get_id(node) == "int":
                    ...
                    node.id = "BigFloat"
        elif self._use_arbitrary_precision and ann == "int":
            if get_id(node) == "int":
                node.id = "BigInt"
            elif get_id(node) == "float":
                node.id = "BigFloat"
    ...
    def _generic_assign_visit(self, node: Union[ast.Assign, ast.AnnAssign], target):
        ...
        if annotation:
            if (
                annotation == "BigInt"
                or annotation == "BigFloat"
                or (
                    self._use_arbitrary_precision
                    and (annotation == "int" or annotation == "float")
                )
    ...
    def visit_BinOp(self, node: ast.BinOp) -> Any:
        self.generic_visit(node)
        node.ignore_wrap = getattr(
            node.left, "is_arbitrary_precision_var", False
        ) or getattr(node.right, "is_arbitrary_precision_var", False)
        return node
...
class JuliaNestingRemoval(ast.NodeTransformer):
    ...
    def visit_Module(self, node: ast.Module) -> Any:
        ...
        self._remove_nested_resumables = getattr(
            node, REMOVE_NESTED_RESUMABLES, FLAG_DEFAULTS[REMOVE_NESTED_RESUMABLES]
        )
        body = []
        ...
        for n in node.body:
            if isinstance(n, ast.FunctionDef):
                if is_generator(n) and self._remove_nested_resumables:
                    ...
                    if (
                        resumable_dec
                        and REMOVE_NESTED in resumable_dec
                ...
                REMOVE_NESTED in n.parsed_decorators or self._remove_nested
            ):
...
class JuliaImportRewriter(ast.NodeTransformer):
    ...
    def visit_Attribute(self, node: ast.Attribute) -> Any:
        ...
        assigned_from = (
            val_node.assigned_from if hasattr(val_node, "assigned_from") else None
        )
        find_node = lambda x: isinstance(x, ast.FunctionDef) and x.name == node.attr
        if (
            assigned_from
            and isinstance(assigned_from, (ast.Assign, ast.AnnAssign))
            and isinstance(assigned_from.value, ast.Call)
            and isinstance(
                node.scopes.find(get_id(assigned_from.value.func)), ast.ClassDef
            )
        ):
            class_scope = node.scopes.find(get_id(assigned_from.value.func))
            if isinstance(class_scope, ast.ClassDef) and find_in_body(
                class_scope.body, find_node
            ):
    ...
    def _generic_import_visit(self, node, key="default"):
        if self._import_rewrite:
            ...
            for alias in node.names:
                n = alias.name
                if (
                    n in self._class_import_funcs
                    and self._class_import_funcs[n] not in node.names
                ):
...
class JuliaClassWrapper(ast.NodeTransformer):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> Any:
        ...
        ann = ast.Subscript(
            ...
            slice=ast.Tuple(
                elts=[ast.Name(id="Symbol"), ast.Name(id="Any")], is_annotation=True
            ),
            is_annotation=True,
        )
        if self._has_dict:
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if re.match(r"^super", func_id) is not None:
            ...
            node.func = base
        elif isinstance(node.func, ast.Attribute) and re.match(
            r"^.*__init__$", func_id
        ):
    ...
    def visit_Assign(self, node: ast.Assign) -> Any:
        ...
        if (
            ...
            and get_id(target.value) == "self.__dict__"
        ):
            self._has_dict = True
...
class JuliaClassOOPRewriter(ast.NodeTransformer):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> Any:
        # Add OO decorator
        decorator = ast.Call(
            func=ast.Name(id=OOP_CLASS), args=[], keywords=[], scopes=node.scopes
        )
        keywords = None
        if self._oop_nested_funcs:
            decorator.keywords.append(
                ast.keyword(arg="oop_nested_funcs", value=ast.Constant(value=True))
            )
...
class JuliaClassSubtypingRewriter(ast.NodeTransformer):
    ...
    def __init__(self) -> None:
        ...
        self._ignored_module_set = (
            self._ignored_module_set
        ) = IGNORED_MODULE_SET.copy().union(JL_IGNORED_MODULE_SET.copy())
        self._hierarchy_map = {}
        self._class_scopes = []
    ...
    def visit_Module(self, node: ast.Module) -> Any:
        ...
        for class_name, extends in self._hierarchy_map.items():
            ...
            extends_node = None
            core_module = extends.split(".")[0] if extends else None
            if (
                extends
                and core_module not in self._ignored_module_set
                and extends not in self.IGNORE_ABSTRACT_SET
                and extends in self._hierarchy_map
            ):
        ...
        if abstract_types:
            body = body[: node.import_cnt] + abstract_types + body[node.import_cnt :]
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> Any:
        ...
        if len(node.bases) == 1:
            ...
            raise Exception(
                "Multiple inheritance is only supported with ObjectOriented.jl"
            )
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> Any:
        ...
        for arg in args.args:
            if (
                ...
                and is_class_or_module(annotation, node.scopes)
                and not get_id(annotation) in self.SPECIAL_EXTENDS_MAP
            ):
        ...
        if (
            ...
            and is_class_or_module(node.self_type, node.scopes)
            and not (
                parent_node.bases
                and get_id(parent_node.bases[0]) in self.SPECIAL_EXTENDS_MAP
            )
        ):
...
class VariableScopeRewriter(ast.NodeTransformer):
    ...
    def visit_For(self, node: ast.For) -> Any:
        ...
        if target_id in all_variables_out_of_scope:
            ...
            target = ast.Name(id=target_id, annotation=annotation)
            new_loop_id = f"_{target_id}"
            new_var_assign = ast.Assign(
                targets=[target],
                value=ast.Name(id=new_loop_id, annotation=annotation),
                # annotation = annotation,
...
class JuliaOffsetArrayRewriter(ast.NodeTransformer):
    ...
    SUPPORTED_OPERATIONS = set(["append", "clear", "extend", "len", "range"])
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> Any:
        ...
        for arg in node.args.args:
            ...
            if (
                not hasattr(arg, "annotation")
                or (hasattr(arg, "annotation") and not is_list)
                or arg_id not in self._subscript_vals
            ):
                continue
            arg_name = ast.Name(id=arg_id)
            val = self._build_offset_array_call(
                arg_name, arg.annotation, node.lineno, node.col_offset, node.scopes
            )
            assign = ast.Assign(
                targets=[arg_name],
                value=val,
        ...
        # Add to decorators
        if not (OFFSET_ARRAYS in parsed_decorators) and (
            let_assignments or self._list_assigns
        ):
        ...
        if self._use_offset_array:
            if self._is_assign_val:
                return self._build_offset_array_call(
                    node, node.annotation, node.lineno, node.col_offset, node.scopes
                )
        return node
    ...
    def visit_Assign(self, node: ast.Assign) -> Any:
        ...
        if self._use_offset_array and is_list(node.value):
            ...
            node.value = self._build_offset_array_call(
                node.value, ann, node.lineno, node.col_offset, node.scopes
            )
            self.generic_visit(node)
        else:
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign) -> Any:
        ...
        if self._use_offset_array and is_list(node.value):
            ...
            node.value = self._build_offset_array_call(
                node.value, node.annotation, node.lineno, node.col_offset, node.scopes
            )
            self.generic_visit(node)
        else:
            self._is_assign_val = True
    ...
    def visit_Subscript(self, node: ast.Subscript) -> Any:
        ...
        if self._use_offset_array and (id := get_id(node.value)) and is_list:
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if args and get_id(args[0]) == "sys" and get_id(node.func) == "argv":
    ...
    def visit_Name(self, node: ast.Name) -> Any:
        ...
        if require_parent and get_id(node) in self._list_assigns:
            return self._create_parent(
                node, node.lineno, getattr(node, "col_offset", None)
            )
        return node

    def _build_offset_array_call(
        self, list_arg, annotation, lineno, col_offset, scopes
    ):
        return ast.Call(
...
class JuliaCtypesRewriter(ast.NodeTransformer):
    ...
    def _ctypes_assign_visit(self, node, target) -> Any:
        ...
        admissible_args = (
            re.match(r".*argtypes$|.*restype$|.*errcheck$", get_id(target))
            if get_id(target)
            else False
        )
        t_node = None
        ...
        if not annotation or not admissible_args:
            return node

        if (
            get_id(annotation) in {"ctypes.CDLL", "CDLL", "ctypes.WinDLL", "WinDLL"}
            ...
            if (
                ...
                and get_id(target.value.value.value) == "self"
            ):
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if isinstance(node.func, ast.Attribute) and mod_name:
            if isinstance(node.func.value, ast.Attribute):
                func = ast.Attribute(
                    value=ast.Name(id=node.func.value.attr), attr=node.func.attr
                )
            else:
        ...
        if (
            class_for_typename(get_id(func), None, self._imported_names)
        ...
        if isinstance(func, ast.Attribute):
            ...
            if isinstance(func.value, ast.Name):
                ...
                and get_id(func.value.value) == "self"
            ):
                class_node = find_node_by_type(ast.ClassDef, node.scopes)
                dll_node = class_node.scopes.find(func.value.attr)
                module_name = func.value.attr

        if get_id(getattr(dll_node, "annotation", None)) in {
            "ctypes.CDLL",
            "CDLL",
            "ctypes.WinDLL",
            "WinDLL",
            ...
            if (
                module_name in self._ext_modules
                and named_func in self._ext_modules[module_name]
            ):
            ...
            if (
                self._use_modules
                and (id := f"{mod_name}.{get_id(func)}") in self._factory_funcs
            ):
            ...
            # Retrieve args
            get_field = (
                lambda x: node.args[func_factory[x]]
        ...
        if ccall_func is not None and argtypes is not None:
            ...
            if isinstance(argtypes, ast.Constant) and argtypes.value is None:
            ...
            argtypes.elts = list(
                map(lambda x: ptr_node if replace_cond(x) else x, argtypes.elts)
            )
            ...
            ccall_assign = ast.Assign(targets=[ast.Name(id="res")], value=ccall)
            # Build error check call
            errcheck_call: ast.Call = (
                self._build_errcheck_call(node, errcheck, ccall_func, restype)
                if errcheck
                else None
            )
            func_name = (
                get_id(ccall_func.args[1])
            ...
            if is_factory:
                ...
                args = ast.arguments(
                    args=[ast.arg(arg=var) for var in var_list], defaults=[]
                )
                annotations = list(
                    map(lambda x: getattr(x, "annotation", None), old_argtypes)
                )
    ...
    def _make_ptr(self, ptr_type: str):
        ptr_node = ast.Subscript(
            value=ast.Name(id="Ptr"), slice=ast.Name(id=ptr_type), is_annotation=True
        )
        ast.fix_missing_locations(ptr_node)
        return ptr_node

    def _build_errcheck_call(
        self, node, errcheck, ccall_func: ast.Call, restype
    ) -> ast.Call:
        ...
        if not argtypes:
            ...
            for arg in node.args:
                if getattr(arg, "annotation", None):
                    ...
                    and class_for_typename(get_id(arg.func), None, self._imported_names)
                    in self.CTYPES_LIST
                ):
                    argtypes_lst.append(arg.func)
                elif isinstance(arg, ast.Call) and get_id(arg.func) == "ctypes.cast":
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> Any:
        ...
        if (
            return_type in named_func_ptr_types
            and func_ptr is not None
            and argtypes is not None
            and restype is not None
        ):
    ...
    def visit_Attribute(self, node: ast.Attribute) -> Any:
        ...
        if isinstance(node.value, ast.Attribute) and get_id(
            getattr(node.value, "annotation", None)
    ...
    def _build_libdl_call(self, scopes):
        libdl_call = ast.Attribute(
            value=ast.Name(id="Libdl"), attr="dlsym", ctx=ast.Load(), scopes=scopes
        )
        ast.fix_missing_locations(libdl_call)
        return libdl_call
    ...
    def _remove_deleted_funcs(self, node, module=None):
        remove_del_funcs = (
            lambda x: f"{module}.{x.name}" not in self._factory_funcs
            if module
            else x.name not in self._factory_funcs
        )
...
class JuliaCtypesCallbackRewriter(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> Any:
        ...
        # node.body = body
        node.returns = (
            None  # Returns are unreliable, as functions are used as callbacks
        )
        return node
    ...
    def visit_Return(self, node: ast.Return) -> Any:
        ...
        if self._is_callback:
            ...
            if return_node_ann in self.CONVERSION_MAP:
                conv_type = ast.Name(
                    id=self.CONVERSION_MAP[return_node_ann], preserve_keyword=True
                )
...
class JuliaArgumentParserRewriter(ast.NodeTransformer):
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        if (
            ...
            if (
                get_id(getattr(argparse_node, "annotation", None))
...
class JuliaExceptionRewriter(ast.NodeTransformer):
    ERROR_FUNCTIONS = {"WindowsError": ["function", "winerror"]}  # "strerror"
    ...
    def visit_Name(self, node: ast.Name) -> Any:
        node_id = node.id
        if node_id in self._exceptions and not getattr(node, "is_attr", None):
    ...
    def _generic_assign_visit(self, node, target):
        ...
        if (
            isinstance(node.value, ast.Call)
            and get_id(node.value.func) in self.ERROR_FUNCTIONS
        ):
            if isinstance(target, ast.Name):
                err_name = get_id(node.value.func)
                self._exceptions[get_id(target)] = {"err_name": err_name}
                for arg in self.ERROR_FUNCTIONS[err_name]:
                    self._exceptions[get_id(target)][arg] = None
                return None
        elif (
            isinstance(target, ast.Attribute)
            and get_id(target.value) in self._exceptions
        ):
